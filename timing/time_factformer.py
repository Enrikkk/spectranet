"""Inference timing for FactFormer2D. With max_latent_steps=10=T_out, it's
a single model() call yielding all 10 frames (non-AR in practice)."""
import argparse
import math
import os
import sys
...
# Reconstruct FactFormer2D verbatim from ns_factformer.py — needed because
# the trainer defines the class inline rather than importing.
...
class FactFormer2D(nn.Module):
    """Verbatim from Baselines/FactFormer/ns_factformer.py — checkpoint-shape
    must match exactly, hence biases/groups/Conv1d depth as in the trainer."""
...
def main():
    ap = argparse.ArgumentParser('FactFormer inference timing')
    ap.add_argument('--checkpoint', default=f"{FF_DIR}/checkpoints/ns_factformer_our_split_best.pt")
    ap.add_argument('--output_csv', default=f"{THIS_DIR}/../results/FactFormer_timing.csv")
    ...
    # paper defaults from ns_factformer.py argparse
    ...
    def forward_fn(B: int):
        ...
        # eval pattern: max_latent_steps == T_out == 10 → one call
