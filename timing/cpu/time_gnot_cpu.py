...
from paths_cpu import GN_DIR, RESULTS_DIR  # noqa: E402
...
def _make_batched_graph(B: int, N: int, T_in: int, device) -> "dgl.DGLGraph":
    ...
    for _ in range(B):
        ...
        g.ndata['x'] = torch.cat([coords_t, u_in], dim=-1)
...
def main():
    ap = argparse.ArgumentParser('GNOT CPU inference timing')
    ap.add_argument('--checkpoint', default=os.path.join(GN_DIR, "checkpoints", "ns_gnot_our_split_best.pt"))
    ap.add_argument('--output_csv', default=os.path.join(RESULTS_DIR, "GNOT_timing.csv"))
    ...
    model = CGPTNO(
        trunk_size=input_dim + 1, branch_sizes=None, output_size=output_dim,
        n_layers=3, n_hidden=128, n_head=1, attn_type='linear',
        ffn_dropout=0.0, attn_dropout=0.0, mlp_layers=3,
        act='gelu', horiz_fourier_dim=0,
    ).to(device).eval()
