"""Aggregate per-model timing CSVs and produce six plots.

Reads ./data/FNO/TIME_ANALYSIS/results/*_timing.csv, joins with the
hard-coded test L2 leaderboard, writes leaderboard_speed.csv, and emits
six plots into ../plots/.
"""
import csv
import glob
import math
...
RESULTS  = os.path.normpath(os.path.join(THIS_DIR, "..", "results"))
PLOTS    = os.path.normpath(os.path.join(THIS_DIR, "..", "plots"))
...
def load_rows():
    rows = []
    for path in sorted(glob.glob(os.path.join(RESULTS, "*_timing.csv"))):
        with open(path) as f:
            rd = csv.DictReader(f)
            for r in rd:
...
def write_leaderboard(rows):
    out = os.path.join(RESULTS, "leaderboard_speed.csv")
...
def plot_latency_bar(rows, B, fname, title):
    ...
    for i, (y, m, e_lo, e_hi, c, st) in enumerate(zip(ypos, med, err[0], err[1], colors, statuses)):
        if st == "OOM":
            ax.barh(y, 1, color="white", edgecolor=c, hatch="///",
    ...
    ax.invert_yaxis()
    ax.set_xlabel("latency per 10-step prediction (ms)")
    ax.set_title(title)
    # legend by category
    seen = []
    handles = []
    ...
    fig.tight_layout()
    fig.savefig(os.path.join(PLOTS, fname), dpi=140)
    plt.close(fig)
    print(f"wrote {fname}")
...
def plot_params_vs_latency(rows):
    ...
    ax.set_ylabel("latency at batch=1 (ms, log)")
    ax.set_title("params vs latency  —  full attention is above the param-fit line")
    ax.grid(True, which="both", alpha=0.3)
    fig.tight_layout()
    fig.savefig(os.path.join(PLOTS, "params_vs_latency.png"), dpi=140)
    plt.close(fig)
...
def plot_l2_vs_latency(rows):
    ...
    # Pareto frontier (lower-left optimal): minimal latency at each L2 ceiling
    ...
    ax.set_xscale("log")
    ax.set_xlabel("latency at batch=1 (ms, log)  —  10-step prediction")
    ax.set_ylabel("test L2 (lower = better)")
    ax.set_title("Accuracy vs Inference Cost  —  NS ν=1e-5")
    ax.grid(True, which="both", alpha=0.3)
    fig.tight_layout()
    fig.savefig(os.path.join(PLOTS, "test_l2_vs_latency.png"), dpi=140)
    plt.close(fig)
    print("wrote test_l2_vs_latency.png  (HEADLINE)")


def plot_peak_memory(rows):
    sub = [r for r in _by_batch(rows, 8) if r["status"] == "ok"]
    if not sub:
        return
    ...
    ax.set_xlabel("peak GPU memory (MB) at batch=8")
    ax.set_title("Peak GPU Memory  —  batch=8")
    fig.tight_layout()
    fig.savefig(os.path.join(PLOTS, "peak_memory.png"), dpi=140)
    plt.close(fig)
...
def main():
    rows = load_rows()
    if not rows:
        print(f"no rows found in {RESULTS}")
        return
    write_leaderboard(rows)
    plot_latency_bar(rows, 1, "latency_b1.png",  "Latency  —  batch=1 (10-step prediction)")
    plot_latency_bar(rows, 8, "latency_b8.png",  "Latency  —  batch=8 (10-step prediction)")
    plot_throughput(rows, 32, "throughput_b32.png", "Throughput  —  batch=32")
