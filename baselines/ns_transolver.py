"""
Transolver baseline on NS ν=1e-5 — adapted to our 850/150/200 split.

Original: https://github.com/thuml/Transolver  (exp_ns.py)
Changes:
  ...
  - Default hyperparams = paper's NS settings: n_hidden=256, n_heads=8, n_layers=8, slice_num=32

Run from: ./third_party/Transolver/PDE-Solving-StandardBenchmark/
"""
...
parser = argparse.ArgumentParser('Transolver NS — our split')
...
parser.add_argument('--save_name',    type=str,   default='ns_transolver_our_split')
...
def eval_checkpoint(ckpt_path, model, loader, n_samples, device):
    ...
    total = 0.0
    per   = []
    loss_fn = TestLoss(size_average=False, reduction=False)  # per-sample
    with torch.no_grad():
        for x, fx, yy in loader:
            ...
            per_batch = loss_fn(pred.reshape(bsz, -1), yy.reshape(bsz, -1))  # (bsz,)
...
def main():
    ...
    train_loss_fn = TestLoss(size_average=False)   # returns sum over batch
    ...
    for ep in tqdm(range(args.epochs), desc='Transolver NS'):
        model.train()
        train_full = 0.0
    ...
    print(f'(3D C1 best: 0.1643 | Transolver paper NS: 0.0900)')
