"""
ONO2 (Orthogonal Neural Operator) on NS ν=1e-5 — adapted to our 850/150/200 split.

Original: https://github.com/yaohua32/Orthogonal-Neural-operator  (NS_example2.py)
Changes:
  - ntrain=850, nval=150, ntest=200 (was ntrain=1000, ntest=200)
  - data indices: train[:850], val[850:1000], test[-200:]
  - epochs=200 (was 500)
  - Best-val checkpoint + per-sample test CSV output
  - Default hyperparams = paper's NS settings: n_hidden=64, n_layers=3, n_heads=4, psi_dim=64

Run from: ./third_party/Orthogonal-Neural-operator/
"""
...
parser = argparse.ArgumentParser('ONO2 NS — our split')
...
parser.add_argument('--save_name',    type=str,   default='ns_ono_our_split')
...
def eval_checkpoint(ckpt_path, model, loader, n_samples, device):
    ...
    total = 0.0
    per   = []
    loss_fn = TestLoss(size_average=False, reduction=False)
    with torch.no_grad():
...
def main():
    ...
    model = ONO2(
        n_hidden=args.n_hidden, n_layers=args.n_layers, space_dim=2,
        Time_Input=False, fun_dim=T_in, n_head=args.n_heads,
        momentum=args.momentum, orth=args.orth, psi_dim=args.psi_dim,
        mlp_ratio=args.mlp_ratio, attn_type=args.attn_type
    ).to(device)
    ...
    train_loss_fn = TestLoss(size_average=False)
    val_loss_fn   = TestLoss(size_average=False)
    ...
    for ep in tqdm(range(args.epochs), desc='ONO2 NS'):
        model.train()
        train_full = 0.0
        for x, fx, yy in train_loader:
            ...
            for t in range(0, T, step):
                y    = yy[..., t:t+step]
                im   = model(x, fx=fx)
                loss += train_loss_fn(im.reshape(bsz,-1), y.reshape(bsz,-1))
                pred  = im if pred is None else torch.cat((pred, im), -1)
    ...
    print(f'(Transolver paper ONO NS: 0.1195)')
